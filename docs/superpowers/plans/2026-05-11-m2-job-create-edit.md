...
| F7 | B.6 Step 3 widget test 순서 의존성 | spec/plan에 "B.6 widget test JobCreateScreen `_addPhoto` null 처리" 명시 | **B.8 wire-up이 `_addPhoto` 도입 → B.6 Step 3는 B.8 deliverables로 흡수** (실행 세션 advisor catch). B.6 Step 1+2(plist/manifest) + Step 5(stage)만 본 task로 land, 권한 거부 widget test는 B.8 테스트 묶음에 포함 | B.6 mechanical config-only + B.8 _addPhoto 테스트 케이스 1건 추가 |
| F8 | B.7 구현 파일 위치 정정 | plan §4617 "Modify: `sharework/lib/screens/giver/giver_main_screen.dart`" | **`giver_main_screen.dart`은 5탭 BottomNav 셰(M1 baseline) → 유지**. "내 공고" 4탭 listMine 구현은 **`lib/screens/giver/home/giver_home_screen.dart`** (현재 2탭 진행중/완료 + Dummy.jobs)로 land. 사용자 결정 lock-in (2026-05-12 본 세션 AskUserQuestion (A)). | plan B.7 Files Modify 1건 교체. M1 5탭 입수구 보존, Dummy.jobs → 실 API listMine 전환. _JobList private widget도 GiverHomeScreen 내부에서 함께 정리 |
| F9 | B.11 integration test 위치 정정 | plan §5250 "Create: `sharework/integration_test/m2_giver_smoke_test.dart`" | **`test/integration/m2_giver_smoke_test.dart`** (M1 baseline 동등 위치). `integration_test/` 디렉토리는 본 프로젝트에 없으며, `pubspec.yaml`에 `integration_test` dev_dependency 미등록. M1 smoke (`test/integration/m1_smoke_test.dart`)는 widget-test 기반 mock-driven flow로 land됨 — 동일 패턴 적용. 실 simulator/device 통합 테스트는 M3+에서 도입. | plan B.11 Files Create 경로 변경. `flutter test test/integration/m2_giver_smoke_test.dart`로 실행. `app.main()` 호출 대신 GiverHomeScreen → JobCreateScreen → JobEditScreen 직접 pump |
| F10 | B.10 plan 위젯 "Paused"/"Close" 라벨 정정 | plan §5295 `find.text('Close')` | **JobStatusToggle 실제 ButtonSegment label = `Close`** (`lib/widgets/job_status_toggle.dart:40`). plan 텍스트 정합 ✓. 별도 정정 불필요 — F10은 검증 confirmation row. | B.11 integration test 라벨 일치성 사전 확인 완료 |
